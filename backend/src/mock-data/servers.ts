...
export const SERVER_SPECS: ServerSpec[] = [
  ...
  },
  {
    ...
    clusterId: 'lab-proxmox',
    os: 'Debian 12 · Docker 27',
    description: 'Services host — media + monitoring stack.',
    role: 'docker',
    ip: '10.0.0.11',
    location: 'Rack R1 / U34',
    cpuModel: 'Intel i5-13500 (14C/20T)',
    cpuCores: 20,
    ramTotalGb: 32,
    diskTotalGb: 2048,
    sensors: [
      { kind: 'cpu_temp', label: 'CPU Temperature', unit: '°C', base: 44, variance: 4, correlatesWith: 'temp', correlation: 0.9, warningThreshold: 80, criticalThreshold: 90 },
      { kind: 'fan_rpm', label: 'CPU Fan RPM', unit: 'rpm', base: 860, variance: 70, correlatesWith: 'temp', correlation: 0.75, warningThreshold: 2500, criticalThreshold: 2800 },
      { kind: 'fan_speed', label: 'Fan Duty', unit: '%', base: 30, variance: 8, correlatesWith: 'temp', correlation: 0.7, warningThreshold: 85, criticalThreshold: 95 },
      { kind: 'power_consumption', label: 'Power Draw', unit: 'W', base: 45, variance: 12, correlatesWith: 'cpu', correlation: 0.65, warningThreshold: 180, criticalThreshold: 220 },
      { kind: 'nvme_temp', label: 'NVMe Temp', unit: '°C', base: 40, variance: 4, correlatesWith: 'cpu', correlation: 0.4, warningThreshold: 68, criticalThreshold: 74 },
    ],
    profile: {
      baseCpu: 28,
      cpuAmplitude: 18,
