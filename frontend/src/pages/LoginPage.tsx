...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  server_unreachable:
    'Cannot reach the server. Make sure the backend is running (npm run dev -w backend, or docker compose up).',
};
...
export default function LoginPage() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-base px-4">
      ...
      <div className="pointer-events-none absolute inset-0 opacity-[0.35] [background-image:linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] [background-size:44px_44px]" />
      ...
      >
          ...
          {mode === 'recovery' ? (
            ...
            {!twoFactorToken && (
              <button
                ...
                className="self-center text-xs text-text-muted transition-colors hover:text-text-primary cursor-pointer"
              >
                Forgot password?
              </button>
            )}
        ...
        <div className="mt-4 flex items-center justify-center gap-2">
          <Badge tone="neutral">Public read-only dashboard</Badge>
          <span className="text-[11px] text-text-muted">— configuration requires admin sign-in</span>
        </div>
      </motion.div>
    </div>
  );
